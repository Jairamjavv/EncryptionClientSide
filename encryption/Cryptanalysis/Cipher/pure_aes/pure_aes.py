...
class AES(object):
    ...
    @staticmethod
    def RotWord(word):
        # Cyclic permutation of the word
    ...
    ctr = os.urandom(16)
    def counter(self, iv, ctr):
        self.ctr = ctr + 1
        self.iv = iv
        ctr = os.urandom(16)
        xor_ctr = xor(self.iv, ctr)
        return xor_ctr

    def ctr(self, data, expanded_key, isInv, xor_ctr):
        # Encrypt hex string data
        if self.input is 'hex':
            if not isInv: return self.Cipher(expanded_key, xor_ctr)
            elif isInv: return self.InvCipher(expanded_key, xor_ctr)
        # Encrypt an text string
        elif self.input is 'text':
            if not isInv: return self.Cipher(expanded_key, ''.join('%02x' % x for x in self.pad(xor_ctr.encode())))
            elif isInv: return str(self.unpad(binascii.unhexlify(self.InvCipher(expanded_key, data).encode())))[2:-1]
        # Encrypt a stream of binary data
        elif self.input is 'data':
            if not isInv: return b''.join(binascii.unhexlify(self.Cipher(
